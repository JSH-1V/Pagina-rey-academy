...
export function FaqAccordion({ stagger = false }: { stagger?: boolean }) {
      ...
      {FAQS.map((faq, i) => {
        ...
        return (
          <motion.div
            ...
            initial={stagger ? { opacity: 0, y: 20, filter: "blur(6px)" } : { opacity: 0, y: 20 }}
            whileInView={stagger ? { opacity: 1, y: 0, filter: "blur(0px)" } : { opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={stagger ? { duration: 0.5, delay: i * 0.08, ease: [0.22, 1, 0.36, 1] } : { duration: 0.5, delay: faq.delay }}
          >
            <button
              onClick={() => toggle(i)}
